...
快捷映射页会列出触发键、输出键和中文动作说明，选中一行后可以编辑或删除，也可以一键恢复默认映射。

![按键映射配置](docs/images/mapping-page.png)

新增和编辑映射使用独立表单。触发键填写 `Q`、`H`、`1` 这类原始按键，输出键填写 `B`、`LEFT`、`F1` 这类目标按键。

![新增按键映射](docs/images/mapping-form.png)
...
- 快捷映射支持新增、编辑、删除和恢复默认，保存后立即生效。

## 语音输入与模型

主程序包不包含模型文件。语音模型来自 [FunAudioLLM/SenseVoice](https://github.com/FunAudioLLM/SenseVoice)，已作为单独 release 资产发布。

模型加载成功后，按住 `CapsLock + A` 会显示听写提示窗；松开组合键后，识别结果会自动输入到当前光标位置。

![语音输入提示](docs/images/voice-prompt.png)
